...
#    print(np.shape(img))
    ...
    for i in range(len(sum_x)):
        if sum_x[len(sum_x)-1-i] > 0:
            end_point = len(sum_x)-1-i
            break

    img = np.array(img)[:, start_point:end_point+1]
#    print('after')
#    print(np.shape(img))
    return img
...
#    reX, reY = random.uniform(0.5,1.5),random.uniform(0.5,1.5)
    ...
    for i in range(10):
        ...
        for j in range(10):
            ...
            img = np.loadtxt(random.choice(addrs[label][0]), dtype=np.dtype(np.int16),delimiter=',')
            #img=cv2.imread(random.choice(addrs[label][0]),cv2.IMREAD_GRAYSCALE)
            img_r=cv2.resize(img,None,fx=reX**2+0.5, fy=reY**2+0.5)
            img=crop_image(img_r)
            h = np.shape(img)[0]
            w = np.shape(img)[1]
            if j == 0:
...
#    img_ext[mask[0],mask[1]]=mask_val[mask[0],mask[1]]
    return background, start_x, start_y, width, height, labels
...
#        file_names = [fn for fn in os.listdir(relevant_path) if any(fn.endswith(ext) for ext in included_extensions)]
    ...
    for i in range(num_training):
        ...
        csvarray = np.zeros((cell_num*cell_num, (num_classes+5)*2))
